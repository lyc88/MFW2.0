...
public class ExportExcel<T> {
     ...
     * @throws Exception
     */
    public void exportUserList(HttpServletResponse res, List<SysUser> userList)throws Exception{
        String[] columns = {"登录名", "性别", "邮箱", "手机号", "头像", "创建时间"};
        String title = Constant.EXCEL_TITLE_PREFIX + "_" + DateUtil.getNowShortStringDate() + ".xls";
        ...
        * */
        ...
        headStyle.setAlignment(HorizontalAlignment.CENTER);        //居中显示
        headStyle.setVerticalAlignment(VerticalAlignment.CENTER);  //上下居中

        //设置列头的内容
        HSSFRow columnHead = sheet.createRow(0);
        columnHead.setHeight((short) 800);
        //前半部分
        ...
        for (int i = 0; i < columns.length; i++) {
            HSSFCell cell = columnHead.createCell(i);
        ...
        //循环列样式
        HSSFCellStyle columnsStyle = workbook.createCellStyle();
        columnsStyle.setAlignment(HorizontalAlignment.CENTER);        //水平居中
        columnsStyle.setVerticalAlignment(VerticalAlignment.CENTER);  //上下居中
        HSSFFont columnFont = workbook.createFont();
        columnFont.setFontHeightInPoints((short) 12);
        columnsStyle.setFont(columnFont);
        columnsStyle.setWrapText(true); //自动换行
        for (int i = 0; i < userList.size(); i++) {  //生成表格设置样式、填充值
            SysUser user=userList.get(i);
            HSSFRow boxRow = sheet.createRow(rowNum + 1);
            boxRow.setHeight((short) 800);
            //"登录名",
            HSSFCell cell0 = boxRow.createCell(0);
            cell0.setCellValue(user.getUserName());
            cell0.setCellStyle(columnsStyle);
            // "性别",
            HSSFCell cell1 = boxRow.createCell(1);
            cell1.setCellValue(user.getSex());
            cell1.setCellStyle(columnsStyle);
            //"邮箱",
            HSSFCell cell2 = boxRow.createCell(2);
            cell2.setCellValue(user.getEmail());
            cell2.setCellStyle(columnsStyle);
            // "手机号",
            HSSFCell cell3 = boxRow.createCell(3);
            cell3.setCellStyle(columnsStyle);
            cell3.setCellValue(user.getMobile());
            //  "头像",
            HSSFCell cell4 = boxRow.createCell(4);
            cell4.setCellStyle(columnsStyle);
            cell4.setCellValue(user.getAvatarImg());
            //"创建时间"
            HSSFCell cell5 = boxRow.createCell(5);
            cell5.setCellStyle(columnsStyle);
            cell5.setCellValue(DateUtil.dateToString(user.getCreateTime()));
            rowNum++;
        }
        exportExcel(res, title + ".xls", workbook);
    }
     ...
     * @throws Exception
     */
    public void exportUseAll(HttpServletResponse res, List<ExportUser> userList)throws Exception{
        String[] columns = {"登录名", "手机号","创建时间", "角色編號", "角色名称", "菜单编号", "菜单名称"};
        String title = Constant.EXCEL_TITLE_PREFIX + "_" + DateUtil.getNowShortStringDate() + ".xls";
        //1：创建一个Excel文件
        HSSFWorkbook workbook = new HSSFWorkbook();
        //2：创建一个Excel文件
        ...
        firstfont.setFontHeightInPoints((short) 24);                //设置字体大小
        firstfont.setColor(IndexedColors.BLACK.getIndex());       //设置字体颜色

        //设置列头的样式
        HSSFCellStyle firstHeadStyle = workbook.createCellStyle();
        firstHeadStyle.setFont(firstfont);
        firstHeadStyle.setAlignment(HorizontalAlignment.CENTER);        //居中显示
        firstHeadStyle.setVerticalAlignment(VerticalAlignment.CENTER);
        //设置表头的内容
        //合并单元格CellRangeAddress构造参数依次表示起始行，截至行，起始列， 截至列
        CellRangeAddress firstRegion = new CellRangeAddress(0,0,(short)0,(short)6);
         //该行是取第一行的信息
        HSSFRow firstHead = sheet.createRow(0);
        firstHead.setHeightInPoints(45);
        sheet.addMergedRegion(firstRegion);
        HSSFCell firstHeadCell= firstHead.createCell(0);
        firstHeadCell.setCellValue("用户-角色-菜单");
        firstHeadCell.setCellStyle(firstHeadStyle);

        /**
         * 设置第二行信息：用户主要信息 角色信息，菜单信息
         */
         //表头的字体
        HSSFFont secondFont = workbook.createFont();
        secondFont.setFontHeightInPoints((short) 24);                //设置字体大小
        secondFont.setColor(IndexedColors.BLACK.getIndex());       //设置字体颜色

        //设置列头的样式
        HSSFCellStyle secondHeadStyle = workbook.createCellStyle();
        secondHeadStyle.setFont(secondFont);
        secondHeadStyle.setAlignment(HorizontalAlignment.CENTER);        //居中显示
        secondHeadStyle.setVerticalAlignment(VerticalAlignment.CENTER);  //上下居中

        //设置表头的内容
        //该行是取第二行的信息
        HSSFRow head = sheet.createRow(1);
        //合并单元格CellRangeAddress构造参数依次表示起始行，截至行，起始列， 截至列
        CellRangeAddress region1 = new CellRangeAddress(1,1,(short)0,(short)2);
        head.setHeightInPoints(45);
        sheet.addMergedRegion(region1);
        HSSFCell headcell = head.createCell(0);
        headcell.setCellValue("用户信息");
        headcell.setCellStyle(secondHeadStyle);

        CellRangeAddress region2 = new CellRangeAddress(1,1,(short)3,(short)4);
        sheet.addMergedRegion(region2);
        HSSFCell headcell2 = head.createCell(3);
        headcell2.setCellValue("角色信息");
        headcell2.setCellStyle(secondHeadStyle);

        CellRangeAddress region3 = new CellRangeAddress(1,1,(short)5,(short)6);
        sheet.addMergedRegion(region3);
        HSSFCell headcell3 = head.createCell(5);
        headcell3.setCellValue("菜单信息");
        headcell3.setCellStyle(secondHeadStyle);

        /**
         * 第三行显示columns
         */
        //设置列头的内容
        HSSFRow columnHead = sheet.createRow(2);
        columnHead.setHeight((short) 800);
        ...
        columnHeadFont2.setFontHeightInPoints((short) 12);
        columnHeadFont2.setColor(IndexedColors.BLACK.getIndex());
        cellStyle2.setFont(columnHeadFont2);
        //填充背景颜色
        cellStyle2.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        cellStyle2.setFillForegroundColor(IndexedColors.BLUE.getIndex());//设置背景色颜色
        cellStyle2.setAlignment(HorizontalAlignment.CENTER);        //居中显示
        ...
        columnHeadFont3.setFontHeightInPoints((short) 12);
        columnHeadFont3.setColor(IndexedColors.BLACK.getIndex());
        cellStyle3.setFont(columnHeadFont3);
        //填充背景颜色
        cellStyle3.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        cellStyle3.setFillForegroundColor(IndexedColors.GREEN.getIndex());
        cellStyle3.setAlignment(HorizontalAlignment.CENTER);        //居中显示
        ...
        HSSFFont styleFont = workbook.createFont();
        styleFont.setFontHeightInPoints((short) 12);
        HSSFDataFormat format = workbook.createDataFormat();
        textStyle.setFont(styleFont);
        ...
        //由于第一行，第二行已经被使用，所以从rowNum=2
        int rowNum = 2;
        for (int i = 0; i < userList.size(); i++) {  //生成表格设置样式、填充值
            ExportUser user=userList.get(i);
            HSSFRow boxRow = sheet.createRow(rowNum + 1);
            boxRow.setHeight((short) 800);
            //用户名
            HSSFCell cell0 = boxRow.createCell(0);
            ...
            //用户创建时间
            HSSFCell cell2 = boxRow.createCell(2);
            cell2.setCellValue(user.getCreatTime());
            cell2.setCellStyle(columnsStyle);
            //用户
            HSSFCell cell3= boxRow.createCell(3);
            cell3.setCellValue(user.getRoleCode());
            cell3.setCellStyle(columnsStyle);
            //用户
            HSSFCell cell4 = boxRow.createCell(4);
            cell4.setCellValue(user.getRoleName());
            cell4.setCellStyle(columnsStyle);
            //用户
            HSSFCell cell5 = boxRow.createCell(5);
            cell5.setCellValue(user.getMenuCode());
            cell5.setCellStyle(columnsStyle);
            //用户
            HSSFCell cell6 = boxRow.createCell(6);
            cell6.setCellValue(user.getMenuName());
            cell6.setCellStyle(columnsStyle);
            rowNum++;
        }
        /**
         * 第四行以后显示数据
