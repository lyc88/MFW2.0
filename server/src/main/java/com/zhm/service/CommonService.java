...
public class CommonService {
    ...
    public Result sendSimpleMail(String toEamil, String userName, String subject) {
        ...
        if(sysUsers!=null&&sysUsers.size()>0){
            ...
            return Result.sendFailure("没有查询到用户"+userName);
        }
        //查询是否依旧有数据

        String fromEmail=environment.getProperty("mfw.sendEmail.from.address");
        ...
        try {
            ...
            return  Result.sendSuccess("发送邮件成功",randomCode);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("发送简单邮件时发生异常！", e);
            return Result.sendFailure("发送邮件失败");
        }
    }
