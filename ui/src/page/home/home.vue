<template>
    <div class="home">
        ...
        <div style="float: right;padding-left: 50px;">
            <spn>当前时间：{{nowTime}}</spn>
            <clock :time="myTime"></clock>
        </div>
        <div>
            <h3>日历</h3>
            <el-calendar style="padding: 50px 50px 50px 50px;">
        ...
        <div style="width: 80%">
            ...
            <!--标准柱状图-->
            <div id="standardChart"  :style="{width: '100%', height: '300px'}"></div>

            <!--环形图-->
            <div id="annularChart" :style="{width: '100%', height: '300px'}"></div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            initAnnularChart(){
                let url="api/sysUser/findOperation";
                http.get(url).then(response => {
                        if(response.code==='00000'){
                            // 基于准备好的dom，初始化echarts实例
        ...
        mounted() {
           ...
           this.initStandardChart();
            ...
            this.timer = setInterval(() => {
                _this.nowTime=(new Date()).getFullYear()+"-"+((new Date()).getMonth()+1)+"-"+(new Date()).getDate()+" "+(new Date()).getHours()+":"+(new Date()).getMinutes()+":"+(new Date()).getSeconds();
                _this.myTime = (new Date()).getHours()+":"+(new Date()).getMinutes()+":"+(new Date()).getSeconds(); // 修改数据date
            }, 1000)
        },
        beforeDestroy() {
            if (this.timer) {
